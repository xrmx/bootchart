...
class ProcessTree:
    ...
    def get_start_time(self, process_subtree):
        if not process_subtree:
            return 100000000;
        return min( [min(proc.starttime, self.get_start_time(proc.childList)) for proc in process_subtree] )

    def get_end_time(self, process_subtree):
        if not processSubtree:
            return -100000000;
        return max( [max(proc.starttime + proc.duration, self.get_end_time(proc.childList)) for proc in process_subtree] )

    def get_max_pid(self, process_subtree):
        if not process_subtree:
            return -100000000;
        return max( [max(proc.pid, self.get_max_pid(proc.childList)) for proc in process_subtree] )
