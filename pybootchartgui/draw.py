...
class Draw:
	...
	def cuml_graph(self, proc_tree, chart_bounds, duration, sec_w, stat_type):
		...
		for cs in graph.m_proc_list.values():
			...
			y = last_below = graph.below[last_time]
			last_cuml = cuml = 0.0
			...
			for time in graph.times:
				...
				if graph.below[time] != last_below:
					last_below = graph.below[last_time]
					last_cuml = cuml
					render_seg = True
				...
				if render_seg:
					...
					self.ctx.rectangle (x, graph.below[last_time] - last_cuml, w, last_cuml)
					self.ctx.fill()
					#self.ctx.stroke()
					last_time = time
					y = graph.below [time] - cuml

				row[time] = y

			# render the last segment
			x = chart_bounds[0] + round((last_time - proc_tree.start_time) * chart_bounds[2] / proc_tree.duration)
			y = graph.below[last_time] - cuml
