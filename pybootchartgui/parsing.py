...
def _parse_proc_disk_stat_log(file, options, numCpu):
    ...
    not sda1, sda2 etc, unless show_partitions is True.
    ...
    def is_relevant_line(linetokens, regex_re):
        if len(linetokens) != 14:
            return False
        disk = linetokens[2]
        return regex_re.match(disk)

    disk_stat_samples = []

    for time, lines in _parse_timed_blocks(file):
        sample = DiskStatSample(time)
        relevant_tokens = [linetokens for linetokens in map (lambda x: x.split(),lines)
                           if is_relevant_line(linetokens,
                                               disk_regex_re)]
    ...
    return [DiskSamples("Sum over all disks and partitions", delta_disk_samples(disk_stat_samples, numCpu))]
